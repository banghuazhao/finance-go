//
//  StockGradeViewController.swift
...
class StockGradeViewController: FGUIViewController {
    #if !targetEnvironment(macCatalyst)
    ...
    lazy var tableView = UITableView().then { tv in
        ...
        tv.delegate = self
        tv.dataSource = self
        tv.register(StockGradeCell.self, forCellReuseIdentifier: "StockGradeCell")
...
extension StockGradeViewController {
    @objc private func fetchDataOnline() {
        startFetchData()
        guard let company = company else { return }
        let urlString = APIManager.baseURL + "/api/v3/grade/\(company.symbol)"
        let parameters = [
            "limit": "100",
            ...
            case let .success(data):
                DispatchQueue.background {
                    ...
                    for dict in json.arrayValue {
                        temp.append(StockGrade(dict: dict))
                    }

                    self.stockGrades = temp

                } completion: {
                    self.tableView.reloadData()
                    self.tableView.mj_header?.endRefreshing()
                    if self.stockGrades.count == 0 {
                        self.showNoData()
                    }
                }
    ...
    @objc func share(_ sender: UIBarButtonItem) {
        guard let company = company else { return }
        guard stockGrades.count > 0 else { return }
        ...
        for stockGrade in stockGrades {
            str += "\("Grading Company".localized()): \(stockGrade.gradingCompany)\n"
            if let dateString = Date.convertStringyyyyMMddToDate(string: stockGrade.date)?.stringDayyyyyMMdd {
                str += "\("Date".localized()): \(dateString)\n"
            } else {
                str += "\("Date".localized()): \(stockGrade.date)\n"
            }
            str += "\("Grade".localized()): \("\(stockGrade.previousGrade) to \(stockGrade.newGrade)")\n"
            str += "\("Upgrades & Downgrades".localized()): \(stockGrade.upOrDown)\n"
            str += "\n"
        }

        let file = getDocumentsDirectory().appendingPathComponent("\("Upgrades & Downgrades".localized()) \(company.symbol).txt")
...
extension StockGradeViewController: UITableViewDelegate, UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return stockGrades.count
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "StockGradeCell", for: indexPath) as! StockGradeCell
        cell.stockGrade = stockGrades[indexPath.row]
        return cell
    }
}
