//
//  SingleStockSplitCalendarCell.swift
...
class SingleStockSplitCalendarCell: UITableViewCell {
    var stockSplitCalendar: StockSplitCalendar? {
        didSet {
            ...
            companySymbolLabel.text = stockSplitCalendar.symbol
            let companyName = CompanyStore.shared.companyName(symbol: stockSplitCalendar.symbol)
            ...
            ratioLabel.text = stockSplitCalendar.stockSplitRatio
        }
    }
    
    var searchText: String = "" {
        didSet {
            if let symbol = stockSplitCalendar?.symbol {
            ...
            if let companyCode = stockSplitCalendar?.symbol {
    ...
    lazy var ratioLabel = UILabel().then { label in
        label.adjustsFontSizeToFitWidth = true
        label.numberOfLines = 1
