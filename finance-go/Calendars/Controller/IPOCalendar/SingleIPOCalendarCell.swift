...
class SingleIPOCalendarCell: UITableViewCell {
    var ipoCalendar: IPOCalendar? {
        didSet {
            ...
            companySymbolLabel.text = ipoCalendar.symbol
            let companyName = CompanyStore.shared.companyName(symbol: ipoCalendar.symbol)
            ...
            priceRangeLabel.text = ipoCalendar.priceRange
        }
    }
    
    var searchText: String = "" {
        didSet {
            if let symbol = ipoCalendar?.symbol {
            ...
            if let companyCode = ipoCalendar?.symbol {
    ...
    lazy var priceRangeLabel = UILabel().then { label in
        label.adjustsFontSizeToFitWidth = true
        label.numberOfLines = 1
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        stackView.addArrangedSubview(priceRangeLabel)
