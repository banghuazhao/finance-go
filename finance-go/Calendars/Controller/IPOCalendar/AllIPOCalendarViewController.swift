//
//  AllIPOCalendarViewController.swift
...
class AllIPOCalendarViewController: FGUIViewController {
    #if !targetEnvironment(macCatalyst)
    ...
    var ipoCalendarGroups = [[IPOCalendar]]()
    var dates = [Date]()
    var searchIPOCalendars = [IPOCalendar]()
...
extension AllIPOCalendarViewController {
    @objc func share(_ sender: UIBarButtonItem) {
        var str: String = ""
        var fileName: String = ""

        str += "\("Symbol".localized())\t\("Company".localized())\t\("Price Range".localized())\t\("Date".localized())\n"

        for ipoCalendar in ipoCalendars {
            let symbol = ipoCalendar.symbol
            let name = CompanyStore.shared.companyName(symbol: symbol)
            let priceRange = ipoCalendar.priceRange
            let date = ipoCalendar.date
            str += "\(symbol)\t\(name)\t\(priceRange)\t\(date)\n"
        }

        fileName = "\("IPO Calendar".localized()).txt"
        ...
        NetworkManager.shared.request(
            ...
            case let .success(data):
                DispatchQueue.background {
                    ...
                    self.ipoCalendars = IPOCalendarStore.shared.items

                    let formatter = DateFormatter()
                    formatter.dateFormat = "yyyy-MM-dd"

                    var tempDates = [Date]()
                    tempDates = self.ipoCalendars.map { ipoCalendar in
                        if let date = formatter.date(from: ipoCalendar.date) {
                            return date
                        }
                        return Date()
                    }
                    ...
                    var tempIpoCalendarGroups = [[IPOCalendar]]()
                    for date in tempDates {
                        tempIpoCalendarGroups.append(IPOCalendarStore.shared.items(atDate: date))
                    }
                    self.dates = tempDates
                    self.ipoCalendarGroups = tempIpoCalendarGroups
                } completion: {
...
extension AllIPOCalendarViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if section == 0 {
            ...
            if isSearching {
                return searchIPOCalendars.count
            } else {
                return ipoCalendarGroups[section - 1].count
            }
        }
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.section == 0 {
            ...
            if isSearching {
                ...
                cell.ipoCalendar = searchIPOCalendars[indexPath.row]
                cell.searchText = searchText
                return cell
            } else {
                let cell = tableView.dequeueReusableCell(withIdentifier: "SingleIPOCalendarCell", for: indexPath) as! SingleIPOCalendarCell
                cell.ipoCalendar = ipoCalendarGroups[indexPath.section - 1][indexPath.row]
                cell.searchText = searchText
                return cell
            }
        }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.section != 0 {
            ...
            if isSearching {
                calendarShowViewController.calendar = searchIPOCalendars[indexPath.row]
            } else {
                calendarShowViewController.calendar = ipoCalendarGroups[indexPath.section - 1][indexPath.row]
            }
...
extension AllIPOCalendarViewController: UISearchBarDelegate {
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        self.searchText = searchText
        if searchText != "" {
            let searchTextLower = searchText.lowercased()
            var temp = IPOCalendarStore.shared.items.filter({ (ipoCalendar) -> Bool in
                let symbol = ipoCalendar.symbol
