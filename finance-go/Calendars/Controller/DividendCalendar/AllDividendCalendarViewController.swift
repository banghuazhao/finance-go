//
//  AllDividendCalendarViewController.swift
...
class AllDividendCalendarViewController: FGUIViewController {
    #if !targetEnvironment(macCatalyst)
    ...
    var dividendCalendarGroups = [[DividendCalendar]]()
    var dates = [Date]()
    var searchDividendCalendars = [DividendCalendar]()
    ...
    lazy var tableView = UITableView().then { tv in
        ...
        tv.delegate = self
        tv.dataSource = self
        tv.register(SingleDividendCalendarTitleCell.self, forCellReuseIdentifier: "SingleDividendCalendarTitleCell")
        tv.register(SingleDividendCalendarCell.self, forCellReuseIdentifier: "SingleDividendCalendarCell")
...
extension AllDividendCalendarViewController {
    @objc func share(_ sender: UIBarButtonItem) {
        var str: String = ""
        var fileName: String = ""

        str += "\("Symbol".localized())\t\("Company".localized())\t\("Dividend".localized())\t\("Date".localized())\n"

        for dividendCalendar in dividendCalendars {
            let symbol = dividendCalendar.symbol
            let name = CompanyStore.shared.companyName(symbol: symbol)
            var dividend: String
            if let dividendTemp = dividendCalendar.dividend {
                dividend = String(format: "%.4f", dividendTemp)
            } else {
                dividend = "-"
            }
            let date = dividendCalendar.date
            str += "\(symbol)\t\(name)\t\(dividend)\t\(date)\n"
        }

        fileName = "\("Dividend Calendar".localized()).txt"
...
extension AllDividendCalendarViewController {
    private func fetchDataOnline() {
        let urlString = APIManager.baseURL + "/api/v3/stock_dividend_calendar"
            ...
            case let .success(data):
                DispatchQueue.background {
                    ...
                    for value in json.arrayValue {
                        let calendard = DividendCalendar(json: value)
                        calendards.append(calendard)
                    }
                    DividendCalendarStore.shared.resetItems(dividendCalendars: calendards)
                    self.dividendCalendars = DividendCalendarStore.shared.items

                    let formatter = DateFormatter()
                    formatter.dateFormat = "yyyy-MM-dd"

                    var tempDates = [Date]()
                    tempDates = self.dividendCalendars.map { dividendCalendar in
                        if let date = formatter.date(from: dividendCalendar.date) {
                            return date
                        }
                        return Date()
                    }
                    ...
                    var tempDividendCalendarGroups = [[DividendCalendar]]()
                    for date in tempDates {
                        tempDividendCalendarGroups.append(DividendCalendarStore.shared.items(atDate: date))
                    }
                    self.dates = tempDates
                    self.dividendCalendarGroups = tempDividendCalendarGroups
                } completion: {
...
extension AllDividendCalendarViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if section == 0 {
            ...
            if isSearching {
                return searchDividendCalendars.count
            } else {
                return dividendCalendarGroups[section - 1].count
            }
        }
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.section == 0 {
            let cell = tableView.dequeueReusableCell(withIdentifier: "SingleDividendCalendarTitleCell", for: indexPath) as! SingleDividendCalendarTitleCell
            return cell
        } else {
            if isSearching {
                let cell = tableView.dequeueReusableCell(withIdentifier: "SingleDividendCalendarCell", for: indexPath) as! SingleDividendCalendarCell
                cell.dividendCalendar = searchDividendCalendars[indexPath.row]
                cell.searchText = searchText
                return cell
            } else {
                let cell = tableView.dequeueReusableCell(withIdentifier: "SingleDividendCalendarCell", for: indexPath) as! SingleDividendCalendarCell
                cell.dividendCalendar = dividendCalendarGroups[indexPath.section - 1][indexPath.row]
                cell.searchText = searchText
                return cell
            }
        }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.section != 0 {
            ...
            if isSearching {
                calendarShowViewController.calendar = searchDividendCalendars[indexPath.row]
            } else {
                calendarShowViewController.calendar = dividendCalendarGroups[indexPath.section - 1][indexPath.row]
            }
...
extension AllDividendCalendarViewController: UISearchBarDelegate {
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        self.searchText = searchText
        if searchText != "" {
            let searchTextLower = searchText.lowercased()
            var temp = DividendCalendarStore.shared.items.filter({ (dividendCalendar) -> Bool in
                let symbol = dividendCalendar.symbol
