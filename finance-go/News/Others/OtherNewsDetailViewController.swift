//
//  OtherNewsDetailViewController.swift
...
class OtherNewsDetailViewController: FGUIViewController {
    #if !targetEnvironment(macCatalyst)
    ...
    var selectedStockNew: StockNew? {
        didSet {
            ...
            var otherNewsTitle: String
            var relatedTitle: String
            if newsType == "crypto_news" {
                otherNewsTitle = "Crypto News".localized()
                relatedTitle = "Related Crypto News".localized()
            } else {
                otherNewsTitle = "Forex News".localized()
                relatedTitle = "Related Forex News".localized()
            }
            if selectedStockNew.symbol != "No Data".localized() {
                otherNewsTitle += ": " + selectedStockNew.symbol
            }
            ...
            if let company = CompanyStore.shared.item(symbol: selectedStockNew.symbol) {
                dataSource.append(relatedTitle)
...
extension OtherNewsDetailViewController {
    @objc private func fetchNewsOnline() {
        ...
        let parameters = ["symbol": companySymbol, "apikey": Constants.APIKey]
        NetworkManager.shared.request(urlString: urlString, parameters: parameters, cacheExpire: .seconds(60 * 60), fetchType: .cacheOrOnline) { [weak self] response in
...
extension OtherNewsDetailViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if let object = data as? Company {
            ...
            let stockNewDetailViewController = OtherNewsDetailViewController()
            stockNewDetailViewController.newsType = newsType
...
extension OtherNewsDetailViewController {
    ...
    extension OtherNewsDetailViewController: GADNativeAdLoaderDelegate {
